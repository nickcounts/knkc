// Modify proj8 from chapter 5 so that the departure times are stored in
// an array (and the departure times). Store the times as integers,
// representing the number of minutes since midnight.
//
// Use a loop to search the array of departure times for the one closest
// to the time entered by the user.

// It turns out that I used this approach when I originally solved the
// problem back in chapter 5. I used human readable times for data entry,
// but then immediately translated them to minuts from midnight at the
